<?php
    ...
    if (isset($_GET['grade']) && is_numeric($_GET['grade'])) {
        ...
        $stmt = $conn->prepare("SELECT DISTINCT subjectDescription, subjectAcademicUnit FROM subject WHERE status='active' AND subjectGradeLevel=? ORDER BY subjectDescription ASC");
