...
function _toggleSwitch(id, isAvailable) {
    ...
    return `
        <label class="switch" title="Toggle availability">
            <input
                type="checkbox"
                id="lab-toggle-${_escapeHtml(String(id))}"
                ${checked}
                onchange="toggleAvailability(${_escapeHtml(String(id))})"
            >
...
function _actionButtons(id) {
    return `
        <div class="lab-card__actions">
            <button class="btn-liquid btn-liquid-info" onclick="editLaboratory(${_escapeHtml(String(id))})" title="Edit laboratory">
                <i class="bi bi-pencil me-1"></i>Edit
            </button>
            <button class="btn-liquid btn-liquid-danger" onclick="removeLaboratory(${_escapeHtml(String(id))})" title="Remove laboratory">
...
function _requestButton(id, isAvailable) {
    ...
    return `
        <button
            ...
            onclick="requestLaboratory(${_escapeHtml(String(id))})"
            ${disabled}
...
function removeLaboratory(labId) {
    Swal.fire({
        ...
        if (result.isConfirmed) {
            ...
            // TODO: AJAX delete, then:
            // document.querySelectorAll(`[data-lab-id="${labId}"]`).forEach(el => el.remove());
            // delete _labCardState[labId];
